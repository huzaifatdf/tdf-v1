...
export default function Edit() {
  const { industry, additionalDataStructure, appUrl } = usePage().props;
  const [imagePreview, setImagePreview] = useState(appUrl + '/' + industry.image || null);
  const [thumbnailPreview, setThumbnailPreview] = useState(appUrl + '/' + industry.thumbnail || null);
  ...
  const handleSubmit = useCallback((values, { setSubmitting }) => {
    ...
    router.post(route("industry.update", industry.id), formData, {
      forceFormData: true,
      onSuccess: () => setSubmitting(false),
      onError: () => setSubmitting(false),
    });
  }, [industry.id, additionalDataStructure]);
  ...
  // Create initial values populated with existing industry data
  const createInitialValues = useCallback(() => {
    const initialValues = {
      ...
      thumbnail: industry.thumbnail || null,
      status: industry.status || "",
      priority: industry.priority || 0,
    };

    const existingData = industry.data ? JSON.parse(industry.data) : {};
  ...
  }, [industry, additionalDataStructure]);
  ...
  return (
    <AuthenticatedLayout>
      <Head title={`Edit industry - ${industry.title}`} />
            ...
            >
              {({ isSubmitting, setFieldValue, values }) => (
                <Form className="space-y-6 bg-white p-6 rounded-lg shadow">
                  <div className="mb-6">
                    <h2 className="text-2xl font-bold text-gray-900">Edit industry</h2>
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
                      ...
                      <MediaLibraryModel
                        routename={route('industry.edit', industry.id)}
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => router.get(route("industry.index"))}
                    >
                      Cancel
                    </Button>
