...
const MediaLibraryModel = ({showModal ,setShowModal, routename , setFieldValue ,fieldName,setImagePreview }) => {
  const { collections, media, filters  } = usePage().props;
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            //   copyMediaUrl(item.original_url);
                console.log(item);
                //like that im getting url : http://localhost:8000/storage/16/TDF-Logo-35.png
                // i only need to get the url like this : 16/TDF-Logo-35.png
                const urlParts = item.original_url.split('/');
                const filePath = urlParts.slice(3).join('/'); // Skip the first three parts (http://localhost:8000/storage/)
                setFieldValue(fieldName,filePath);
                setImagePreview(item.original_url);
                setShowModal(false);
            }}
            className="p-2 bg-white rounded-full hover:bg-gray-100"
          >
